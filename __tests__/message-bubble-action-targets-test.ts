...
test('bubble text action slop lifts the ~28dp caption pills to a >=44dp effective touch target', () => {
  // The three shared styles (rawButton/activityToggle/actionButton) floor at
  // minHeight 28; the tallest inner element is caption lineHeight 18
  // (tokens.ts:101). Top/bottom slop 8 each brings the effective target to 44
  // without growing the visible pill.
...
test('every bubble text action applies the helper hitSlop to PressableScale', () => {
  ...
  // Five pressables share the three styles: the tools/thinking activity line
  // (activityToggle), Send again / Retry / Cancel (actionButton) and Raw
