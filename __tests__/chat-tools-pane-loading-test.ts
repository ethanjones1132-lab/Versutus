import { EMPTY_TOOLSETS, applyToolsetsRead, toolsetsListCopy, toolsetsReadFromUnknown } from '@/lib/gateway/toolsets';
...
describe('tools pane loading', () => {
  test('the loading branch holds layout with two Skeleton rows', () => {
    // An opened pane showed a blank scroll while tools.list was in flight
    // (the loading copy is undefined), then popped to full rows when the
    // read landed. The pane now renders skeletons like the sibling panes,
    // so the layout holds instead of jumping.
  ...
  test('the loading copy is undefined, so the blank was real', () => {
    // toolsetsListCopy returns undefined while !loaded && !failed — the only
