import { applySkillsRead, EMPTY_SKILLS, skillsListCopy } from '@/lib/gateway/skills';
...
describe('skills failure keeps its cause', () => {
  test('the fold stores the caught message on a failed first read', () => {
    ...
    // failure arm of SkillsRead now carries the cause into SkillsState.
    const state = applySkillsRead(EMPTY_SKILLS, {
      ok: false,
  ...
  test('the fold stores the cause on a failed re-read and a success clears it', () => {
    const loaded = applySkillsRead(EMPTY_SKILLS, {
      ok: true,
      skills: [{ name: 'weather', description: '' }],
    });
    const stale = applySkillsRead(loaded, { ok: false, error: 'timeout' });
    expect(stale.error).toBe('timeout');
    expect(stale.skills).toEqual(loaded.skills);
    expect(stale.loaded).toBe(true);
    const fresh = applySkillsRead(stale, { ok: true, skills: [] });
  ...
  test('a failure with no message keeps the state honest and messageless', () => {
    ...
    const state = applySkillsRead(EMPTY_SKILLS, { ok: false });
    expect(state.error).toBeUndefined();
    expect(skillsListCopy(state)).toBe('Skills could not be read.');
  });

  test('chat-screen routes caught.message into both skills folds', () => {
    const screen = readSource('src', 'components', 'chat', 'chat-screen.tsx');
    const retry = screen.match(/const handleSkillsRetry[\s\S]*?\}, \[botSurfaceId/)?.[0];
    ...
    expect(screen).toMatch(/applySkillsRead\(previous, \{\s*ok: false,\s*error:/);
    expect(screen).not.toMatch(/applySkillsRead\(previous, \{ ok: false \}\)/);
  });
  ...
  test('the pane renders the kept cause as an ErrorCard with one Retry on a failed first read', () => {
    // The old surface was `skillsListCopy`'s fixed micro line beside a ghost
  ...
  test('keep-working: skeleton window, stale copy, empty copy, retry wiring', () => {
    ...
    ).toBe('Could not re-read skills — showing the last list.');
    const screen = readSource('src', 'components', 'chat', 'chat-screen.tsx');
