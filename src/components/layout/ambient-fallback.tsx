...
/**
 * Reanimated + PNG grain field. Used on web and as the native Skia mount fallback.
 * Sightline plates/orbs take a few px of parallax; grain and vignette stay locked.
 */
export function AmbientFallback({ parallaxX = 0, parallaxY = 0 }: AmbientCanvasProps) {
  const tokens = useTokens();
  const shiftX = parallaxX * 8;
  const shiftY = parallaxY * 8;

  return (
    <View pointerEvents="none" style={StyleSheet.absoluteFill}>
      <View style={{ transform: [{ translateX: shiftX }, { translateY: shiftY }] }}>
        <View
          style={[
            styles.plate,
            styles.plateTop,
