...
export const METHOD_GUIDANCE: Record<string, string> = {
  ...
  'device.info': 'Paired devices are served by the Gate RPC device.list (POST /v1/capabilities/rpc answers {devices}) — /device reads it there when device.info is not dispatched. Versutus devices pair via signed access requests; device tokens stay on the Gate host and are never rendered.',
  'device.repair': 'Paired devices are served by the Gate RPC device.list (POST /v1/capabilities/rpc answers {devices}) — /device repair reads it there instead of an undispatched repair call. Versutus devices pair via signed access requests; device tokens stay on the Gate host and are never rendered.',
  'device.revoke': 'Paired devices are served by the Gate RPC device.list (POST /v1/capabilities/rpc answers {devices}) — /device reads it there for the list view. /device revoke still runs the destructive device.revoke call through its confirmation path. Versutus devices pair via signed access requests; device tokens stay on the Gate host and are never rendered.',
