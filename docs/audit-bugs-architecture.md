...
| P0-3 slash commands unmapped for Hermes | **PARTIALLY FIXED** — pure `rpc-routes.ts` map (17 methods, docs-evidence-based), path-placeholder interpolation (previously broken for `session.get`/`session.messages`), friendly unsupported-method errors, `X-Hermes-Session-Key` header, per-request model override. Still unmapped (no Hermes REST equivalent): `config.*`, `channels.*`, `approvals.*`(list), `env*`, `artifacts.*`, `agents.list`, `devices.*`, `talk.*`, `session.abort/compact` — reported as unsupported instead of failing |
