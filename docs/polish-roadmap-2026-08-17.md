...
- **Progress (2026-08-23):** OpenClaw's private ladder is gone — it now schedules through the shared `ConnectionMonitor` (jittered backoff) in scheduler-only mode (no verified WS health wire yet, so no probe interval). The monitor escalates after `RECONNECT_ESCALATION_ATTEMPTS` (5) consecutive failed retries to an honest `'disconnected'`, which the provider's phase table already maps onto its scheduled auto-retry/re-probe; probe self-heal still wins when the gateway returns. **Remaining:** a verified health wire for the WS dialect (probe + interval), and the user-visible "sleeping" state + manual reconnect affordance.
