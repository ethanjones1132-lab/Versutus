...
- **Progress (2026-08-23):** OpenClaw's private ladder is gone — it now schedules through the shared `ConnectionMonitor` (jittered backoff) in scheduler-only mode (no verified WS health wire yet, so no probe interval). The monitor escalates after `RECONNECT_ESCALATION_ATTEMPTS` (5) consecutive failed retries to an honest `'disconnected'`, which the provider's phase table already maps onto its scheduled auto-retry/re-probe; probe self-heal still wins when the gateway returns. The provider's own auto-retry cool-down no longer probes forever on a flat interval either: every schedule climbs `src/lib/connection/retry-ladder.ts` (12s doubling per consecutive failure, capped at 5 minutes; call-site delays survive as first-attempt floors), forgiving the streak only on a connected phase, a human-initiated retry, or re-enabling auto-connect — the timer-driven cycle deliberately does not reset it. **Remaining:** a verified health wire for the WS dialect (probe + interval), and the user-visible "sleeping" state + manual reconnect affordance.
...
- **Progress (2026-08-24):** First slice DONE at c16fbc9 — completing ANY gateway add now completes onboarding. The two flows were separate URL+token paths where first run bit hardest: the `needsOnboarding` redirect ate fresh-install deep links (`versutus://add?url=…` bounced to generic onboarding with nothing prefilled) and a successful manual add bounced the user back out of chat. `/gateway/add` is now exempt from the redirect (pure `isOnboardingExemptRoute`, jest-pinned), `addGateway` marks onboarding complete + derives the tailnet host for auto-connect probing (`onboardingCompletionForAddedGateway`, jest-pinned), and the deep-link router defers until bootstrap settles. Also verified while here: this section's original premise is partly STALE — `/gateway/setup` IS registered as a titled Stack.Screen and the providers/environments redirect stubs no longer exist on disk.
- **Progress (2026-08-24, second slice) DONE at b7c4541:** capabilities promoted from link-out to a real tab — the editor body moved to `components/gateway/capabilities-section.tsx` (the established `*-section` convention) and the setup hub renders it inline; `/gateway/capabilities` remains as a thin route wrapper so deep links still land on the same editor (no second implementation to drift). Correction (2026-08-24): the rest of this item's old premise is stale — Settings was already slimmed to app-level surfaces (device identity, primary route, runtime diagnostics, transport security) back in 1ea2ae0 (Aug 19), and `GatewayManagementSection` renders only inside the setup hub's Manage tab, so no gateway-list/discovery duplicate survives anywhere. All that remains here is the 2.5-style single-surface Home pass, tracked as its own item below.
...
- **Progress (2026-08-24): core consolidation DONE.** Full survey first: the chat surface mounts **12** sheets (`NewAgentSheet`, `CreateGroupSheet`, `BotDetailSheet`, `PairingSheet`, `SlashCommandPalette`, `ConfirmationSheet`, `ApprovalSheet`, `ChatOverflowSheet`, `BackendPickerSheet`, `MessageActionsSheet`, `ModelPickerSheet`, `SessionSelectorSheet`), all exclusive to `chat-screen.tsx`; `GroupRoomView` adds its own internal rename sheet + leave confirm. Shipped: the three header pickers (sessions / models / backends) are now **one** `ThreadConfigSheet` (`src/components/chat/thread-config-sheet.tsx`) with a segmented control that hops sections without dismissing — the former three sheets survive verbatim as section bodies, their public triggers (title pill, model chip, session chip) unchanged. The host owns no visibility state: `resolveThreadConfigMode` derives the mode from the same provider/local flags as before (pinned by `__tests__/thread-config-test.ts`), so provider-driven opens keep working untouched. Net: the roadmap's original 8-sheet orchestration set is down to **6**. Known tradeoff: section hops re-run the opener's fetch, so the sheet animates close→reopen (identical to the old dismiss-and-retap, just automatic); revisit only if it feels cheap on device.
- **Remaining items — surveyed, deliberately NOT shipped:** (1) *Message actions folding into overflow*: rejected as a literal fold — copy/retry/delete need a per-message anchor, and long-press-sheet is the correct pattern for it; the only true duplicate found anywhere is overflow's "New session" row duplicating the selector's, which can be dropped whenever the overflow is next touched. (2) *Approvals moving to Activity-only with a chat banner*: recommend against — the in-context approval card IS the concierge-pilot wedge moment (send a task from chat, approve where you stand); Activity mirroring the same card is intentional cross-tab redundancy, not drift. (3) *Composer-driven contextual entry* replacing the header chips: optional polish, no operator pain observed; pairing stays as-is (first-run critical). With the picker cluster merged, the "3 sheets max" target is met where it mattered — nothing else merges without hurting the flow, so this item closes here unless device use proves otherwise.
...
- **Progress (2026-08-24): DONE** (found mostly shipped on disk; this pass closed the last surface). `humanizeGatewayError`/`describeGatewayError` (`src/lib/gateway/error-humanizer.ts`, pinned by `error-humanizer-test`) already fed the chat banner and the dashboard's ErrorCard, and every `Alert.alert` call site is gone from `src`. The final raw dump — Home's connect-failure troubleshooting collapsible printing `{lastError}` verbatim — now renders `describeGatewayError(lastError)`, so the first failure a new operator sees reads as product language beside the checklist instead of an exception string.
...
- **Progress (2026-08-24): DONE.** The palette half already shipped (`SlashCommandPalette` + `src/lib/gateway/slash-palette.ts`); this pass closed the residual — channels keep a **persistent subtle status row** directly under the dashboard hero (`describeChannelStatusRow` in `src/lib/gateway/channel-status.ts`, pinned by `__tests__/channel-status-test.ts`, rendered by `components/gateway/channel-status-row.tsx`). Visible whenever the snapshot says anything about channels at all; live-green when ready, warm-accented when degraded, honest ("not confirmed yet", "gateway offline", "offers no channels") otherwise; hidden only when no gateway kind declares channels. It never quotes the group's available/total figures — those tally slash commands, not bridges. Restored en route: 5f5c46e shipped the `dismissGatewayDown` import without ever calling it, so the posted down notice outlived recovery — the call now rides the `clearGatewayDownNotified` decision (the lint dead-import warning had been flagging the gap since Aug 23).
...
- **Progress (2026-08-24):** The stale down-notification now retires itself: `dismissGatewayDown` (notifications/local) dismisses the posted identifier and sweeps the tray by title for notices that outlived an app restart; the provider calls it whenever the connection decision lands back on connected. The in-app reconnect-success signal already exists as the `ConnectedToast` ceremony. The reachability wave is no longer sequential either: wave eligibility + a small concurrency cap live in `src/lib/gateway/reachability-wave.ts` (`planProbeWave`, `runCapped`, cap = 3), so a dashboard verdict lands in ~ceil(N/3) rounds instead of 1.8s × N; pinned by `reachability-wave-test`. The unsupported-shell silent fallback is gone too (2026-08-24, b348335): choosing Shell on a gateway without a terminal endpoint renders an honest EmptyState with a Use-Gateway-RPC action instead of silently substituting RPC; support reads the snapshot lifecycle (`resolveShellSupport`/`describeShellUnavailable` in `src/lib/terminal/shell-support.ts`, pinned by `terminal-shell-support-test`) so warming/partial/offline snapshots report "not confirmed yet" rather than slandering shell-capable gateways. Remaining: onboarding/add merge (tracked with 2.1).
