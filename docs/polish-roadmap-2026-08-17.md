...
- **Progress (2026-08-24, second slice) DONE at b7c4541:** capabilities promoted from link-out to a real tab — the editor body moved to `components/gateway/capabilities-section.tsx` (the established `*-section` convention) and the setup hub renders it inline; `/gateway/capabilities` remains as a thin route wrapper so deep links still land on the same editor (no second implementation to drift). Remaining for 2.1: merge Settings' gateway list into the setup hub's Manage tab and slim Settings to app-level preferences, then the 2.5-style single-surface pass.
